...
Each non-`/healthz` request writes an `access` log with the request URL, cache status, response status, bytes, and duration. It also logs `client_referer`/`client_user_agent` as received by the service, `sent_referer`/`sent_user_agent` as sent to the origin, and `origin_status`/`origin_bytes` as returned by the origin when an origin request is made.
